...
// This is structural metadata (book names + chapter counts), not translation text.
export const BOOKS = [
  // Old Testament
...
export function nextChapterRef(book, chapter) {
  const idx = BOOKS.findIndex((b) => b.name === book);
  if (idx === -1) return null;
  if (chapter < BOOKS[idx].chapters) return { book, chapter: chapter + 1 };
  if (idx < BOOKS.length - 1) return { book: BOOKS[idx + 1].name, chapter: 1 };
  return null;
}

export function prevChapterRef(book, chapter) {
  const idx = BOOKS.findIndex((b) => b.name === book);
  if (idx === -1) return null;
  if (chapter > 1) return { book, chapter: chapter - 1 };
  if (idx > 0) return { book: BOOKS[idx - 1].name, chapter: BOOKS[idx - 1].chapters };
  return null;
}
